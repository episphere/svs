...
svs.loadHashParams = async () => {
	...
	if (hashParams.image && svs.imagesInGCP.includes(hashParams.image)) {
		svs.loadImage(`${svs.gcsBasePath}/${hashParams.image}`)
	}
}

svs.populateSelect = () => {
	const imageSelectElement = document.getElementById("imageSelect")
	svs.imagesInGCP.forEach((image, idx) => {
...
svs.selectImage = () => {
	...
	window.location.hash = `image=${document.getElementById("imageSelect").value}`
}
